...
class Decision(Node):
    ...
    def __init__(self):
        ...
        self.tele_msg = None
        self.wp_msg = None
        self.wp_msg2 = PetConduccion()
        self.emergency_stop_msg = False
        self.override = Override()
        self.telemetry = Telemetry()
        self.telemetry_webots = Telemetry()
    ...
    def pathplanning_callback(self, data: PetConduccion):
        if self.wp_mode == 'wheels':
            data.steering *= self.steering_wheel_conversion
        self.wp_msg = data
        steering = self.wp_msg.steering
        self.wp_msg2 = PetConduccion()
        self.wp_msg2.steering = self.wp_msg.steering
        self.wp_msg2.speed = self.wp_msg.speed
        self.wp_msg2.b_steering =self.wp_msg.b_steering
        self.wp_msg2.b_gear = self.wp_msg.b_gear
        self.wp_msg2.b_brake = self.wp_msg2.b_brake
        self.wp_msg2.b_throttle = self.wp_msg.b_throttle
        self.wp_msg2.id_plataforma = self.wp_msg.id_plataforma
        self.wp_msg2.gear = self.wp_msg.gear

        self.wp_msg.steering = ((self.wp_msg.steering - self.wp_msg.speed) / 0.74) * 4
        self.wp_msg.speed = (steering + self.wp_msg.speed) / 2
    ...
    def decision(self):
        ...
        if self.mode == ModoMision.MANUAL:  # Manual
            ...
            if self.is_valid(self.wp_msg, self.wp_ttl):
                ...
                # else:
                # if self.telemetry_webots.speed < 2:  Limited steering stopped
                #     msg.steering = 0.
        ...
        if self.mode == ModoMision.AUTONOMO:
            vel_simulacion_2 = Twist()
            self.wp_msg2.steering = self.wp_msg2.steering
            vel_simulacion_2.linear.x = self.wp_msg2.speed
            vel_simulacion_2.linear.y = self.wp_msg2.steering
            self.pub_cmd_vel.publish(vel_simulacion_2)

        if self.mode == ModoMision.TELE_OPERADO:
            vel_simulacion = Twist()
            vel_simulacion.linear.x = msg_final.speed - msg_final.steering/150
            vel_simulacion.linear.y = msg_final.speed + msg_final.steering/150
            self.pub_cmd_vel.publish(vel_simulacion)
