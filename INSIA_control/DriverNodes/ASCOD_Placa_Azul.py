...
class CANADACNode(Node):
    ...
    def enable_tension(self, data):
        if data.data:
            msg = make_can_msg(node=self.cobid, index=0x0002, data=0x11, clock=self.get_clock().now().to_msg())
        else:
            msg = make_can_msg(node=self.cobid, index=0x0002, data=0x10, clock=self.get_clock().now().to_msg())
    ...
    def consigna(self, data):
        msg = make_can_msg(node=self.cobid, index=0x0001, data=int(data.data * 1000),
