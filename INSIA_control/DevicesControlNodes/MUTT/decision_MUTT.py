...
class Decision(Node):
    ...
    def __init__(self):
        ...
        self.emergency_stop_lidar_rot_msg = False
        self.override = Override()
        self.telemetry = Telemetry()
        default_mode = ModoMision.TELE_OPERADO
        ...
        self.create_subscription(msg_type=Bool, topic='OverrideEmergencyStop',
                                 ...
                                 qos_profile=HistoryPolicy.KEEP_LAST)
        self.create_subscription(msg_type=Bool, topic='EmergencyStopLidarRot', callback=self.emergency_stop_lidar_rot_callback,
                                 qos_profile=HistoryPolicy.KEEP_LAST)
    ...
    def emergency_stop_lidar_rot_callback(self, data):
        self.emergency_stop_lidar_rot_msg = data.data
    ...
    def follow_me_callback(self, data: PetConduccion):
        ...
        if self.mode == ModoMision.FOLLOW_ME:
            self.timer_control.reset()  
            self.decision()
    ...
    def radio_control_callback(self, data: PetConduccion):
        if self.radio_mode == 'wheels':
            data.steering *= self.steering_wheel_conversion
        self.radio_msg = data
    ...
    def decision(self):
        ...
        if self.mode == ModoMision.MANUAL:  # Manual
            ...
            if self.is_valid(self.tele_msg, self.tele_ttl):
                ...
                msg = self.manual()
        elif self.mode == ModoMision.FOLLOW_ME:  # Follow me
            ...
            if self.is_valid(self.follow_msg, self.follow_ttl):
                ...
                msg = self.manual()
        elif self.mode == ModoMision.RADIO_CONTROL:  # Radio control
            self.logger.debug(f'Modo Radio control')
            if self.is_valid(self.radio_msg, self.follow_ttl):
                msg = self.radio_msg
            else:
                self.logger.debug(f'Msg follow_me is not valid, change to manual')
                msg = self.manual()
        ...
        if self.mode != ModoMision.TELE_OPERADO and self.mode != ModoMision.MANUAL and self.mode != ModoMision.RADIO_CONTROL:
            try:
                if self.emergency_stop_hmi_msg:
                    ...
                    if not self.override_emergency_stop_msg:
                        ...
                        msg_final = self.create_emergency_stop_msg(msg_final, strict_stop=False)
...
class Decision(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(msg_type=Bool, topic='OverrideEmergencyStop', callback=self.override_emergency_stop_callback,
                                 qos_profile=HistoryPolicy.KEEP_LAST)
    ...
    def decision(self):
        ...
        if self.mode == ModoMision.MANUAL:  # Manual
            ...
            if self.is_valid(self.tele_msg, self.tele_ttl):
                ...
                self.logger.debug(f'Msg tele is not valid, change to manual')
                msg = self.manual()
        elif self.mode == ModoMision.FOLLOW_ME:  # Teleoperado
            self.logger.debug(f'Modo Follow_me')
            if self.is_valid(self.follow_msg, self.follow_ttl):
                msg = self.follow_msg
            else:
        ...
        if self.emergency_stop_msg:
         self.logger.error(f'Emergency  {self.emergency_stop_msg = } {self.override_emergency_stop_msg = }')
         if not self.override_emergency_stop_msg:
            msg_final.b_brake = False
            msg_final.b_throttle = False
            msg_final.b_steering = False
            msg_final.b_gear = False
        self.pub_decision.publish(msg_final)
