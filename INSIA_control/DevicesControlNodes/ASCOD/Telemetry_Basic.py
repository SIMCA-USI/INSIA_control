...
from insia_msg.msg import Telemetry2
...
class VehicleNode(Node):
    def __init__(self):
        ...
        self.pub_telemetry = self.create_publisher(msg_type=Telemetry, topic='Telemetry',
                                                   qos_profile=HistoryPolicy.KEEP_LAST)
        self.pub_telemetry_2 = self.create_publisher(msg_type=Telemetry2, topic='Telemetry2',
                                                     qos_profile=HistoryPolicy.KEEP_LAST)

        self.create_subscription(msg_type=CAN, topic='CAN', callback=self.msg_can, qos_profile=HistoryPolicy.KEEP_LAST)
        self.create_subscription(msg_type=EPOSStatus, topic='EPOS4_Freno/Status', callback=self.get_status_brake,
                                 qos_profile=HistoryPolicy.KEEP_LAST)
        self.create_subscription(msg_type=StringStamped, topic='Arduino_Gears/Consigna', callback=self.gear_callback,
                                 qos_profile=HistoryPolicy.KEEP_LAST)

        self.timer_telemetry = self.create_timer(1 / 20, self.publish_telemetry)
        self.timer_heartbeat = self.create_timer(1, self.publish_heartbeat)

    def gear_callback(self, data: StringStamped):
        self.gear = data.data
    ...
    def publish_telemetry(self):
        ...
        self.pub_telemetry_2.publish(msg=self.create_msg_2())
