...
class SteeringNode(Node):
    ...
    def __init__(self):
        ...
        self.pub_target = self.create_publisher(msg_type=Float64, topic='/robot/motors/steering/setpoint',
                                                qos_profile=HistoryPolicy.KEEP_LAST)
    ...
    def controller_update(self, data: PetConduccion):
        """
        Receive data from HL LateralControlPID and transform it in Float stamped
        :param data:
        :return:
        """
        if data.b_steering:
            self.pub_target.publish(Float64(data=data.steering))
        else:
